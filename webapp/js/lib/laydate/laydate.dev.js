...
Dates.timeVoid = function(times, index){
    if(Dates.ymd[1]+1 == Dates.mins[1] && Dates.ymd[2] == Dates.mins[2]){
        if(index === 0 && (times < Dates.mins[3] 
        		|| (times== Dates.mins[3] && Dates.hms[1]< Dates.mins[4])
        		|| (times== Dates.mins[3] && Dates.hms[1]== Dates.mins[4] && Dates.hms[2]< Dates.mins[5]))){
            return 1;
        } else if(index === 1 && Dates.hms[0]== Dates.mins[3]&&( times < Dates.mins[4]
        	|| (times == Dates.mins[4] && Dates.hms[2]< Dates.mins[5]))){
        	return 1;
        } else if(index === 2 && Dates.hms[0]== Dates.mins[3] &&
        		Dates.hms[1]== Dates.mins[4] && times < Dates.mins[5]){
            return 1;
        }
    } else if(Dates.ymd[1]+1 == Dates.maxs[1] && Dates.ymd[2] == Dates.maxs[2]){
        if(index === 0 && (times > Dates.maxs[3]
	        	|| (times== Dates.maxs[3] && Dates.hms[1]> Dates.maxs[4])
	        	|| (times== Dates.maxs[3] && Dates.hms[1]== Dates.maxs[4] && Dates.hms[2]> Dates.maxs[5]))){
            return 1;
        } else if(index === 1 && Dates.hms[0]== Dates.maxs[3] && (times > Dates.maxs[4]
        		|| (times == Dates.maxs[4] && Dates.hms[2]>Dates.maxs[5]))){
            return 1;
        } else if(index === 2 && Dates.hms[0]== Dates.maxs[3] &&
        		Dates.hms[1]== Dates.maxs[4] && times > Dates.maxs[5]){
            return 1;
        }
    }
...
Dates.close = function(){
    ...
    if(Dates.elem.value=='' && doc[byid](Dates.options.explain)){
    	if(doc[byid]('begin').value =='' && doc[byid]('end').value ==''){
    		doc[byid]('begin').style.border ='1px #F00 solid';
    		doc[byid]('begin').style.color ='#F00';
    		doc[byid]('end').style.border ='1px #F00 solid';
    		doc[byid]('end').style.color ='#F00';
    	}else{
    		Dates.elem.style.border ='1px #F00 solid';
    		Dates.elem.style.color ='#F00';
    	}
    	doc[byid](Dates.options.explain).style.color ='#F00';
    	doc[byid](Dates.options.explain).innerHTML=
    		'<i class="ui-tiptext-icon iconfont" title="出错">&#xF045;</i>'+Dates.options.label+ "不能为空";
    }else if(Dates.elem.value!='' && doc[byid](Dates.options.explain)){
    	Dates.elem.style.border ='1px solid #DFD5D5';
    	Dates.elem.style.color ='#595959';
    	if(doc[byid]('begin').value !='' && doc[byid]('end').value !=''){
    		doc[byid](Dates.options.explain).innerHTML='';
    	}
    }
...
Dates.events = function(){
    ...
    log.hmson = function(input, index){
        ...
        Dates.hms = [log.hmsin[0].value, log.hmsin[1].value, log.hmsin[2].value];
...
if ( typeof module === "object" && module && typeof module.exports === "object" ) {
   ...
   if ( typeof define === "function" && define.amd ) {
      define( "laydate", [], function () { return win; } );
   }
}
}(window);
